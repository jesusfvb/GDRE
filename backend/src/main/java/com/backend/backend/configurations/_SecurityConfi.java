...
import com.backend.backend.filters._JwtRequestFilter;
...
@Configuration
@EnableWebSecurity
public class _SecurityConfi extends WebSecurityConfigurerAdapter {

    @Autowired
    _UserDetailsSI serviceUserDetails;

    @Autowired
    _JwtRequestFilter filterJwtRequest;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().authorizeRequests().antMatchers("/login").permitAll().antMatchers("/").permitAll()
                .antMatchers("/swagger-ui/index.html").permitAll().antMatchers("/swagger-ui/swagger-ui.css").permitAll()
                .antMatchers("/swagger-ui/swagger-ui-bundle.js").permitAll()
                .antMatchers("/swagger-ui/swagger-ui-standalone-preset.js").permitAll()
                .antMatchers("/v3/api-docs/swagger-config").permitAll().antMatchers("/v3/api-docs").permitAll()
